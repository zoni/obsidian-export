...
/// It receives:
/// - `context`: The [`Context`] object containing export state and current file information
/// - `reference`: The [`ObsidianNoteReference`] object containing file, section, and label
/// - `is_embed`: `true` for embeds (`![[...]]`), `false` for links (`[[...]]`)
///
...
///
/// **Note:** When a custom handler is set, it becomes responsible for both warning
/// messages and content replacement. No default warnings will be printed.
///
/// # Examples
///
...
/// let mut exporter = Exporter::new(source, destination);
///
/// exporter.missing_note_handler(&|_context, _reference, _is_embed| {
...
/// let mut exporter = Exporter::new(source, destination);
///
/// exporter.missing_note_handler(&|_context, reference, _is_embed| {
///     vec![
...
/// let mut exporter = Exporter::new(source, destination);
///
/// exporter.missing_note_handler(&|_context, reference, is_embed| {
///     if is_embed {
...
/// let mut exporter = Exporter::new(source, destination);
///
/// exporter.missing_note_handler(&|context, reference, is_embed| {
///     // Custom warning logging - handler is responsible for warnings
///     eprintln!(
///         "[CUSTOM] Missing {} '{}' in file: {} (depth: {})",
///         if is_embed { "embed" } else { "link" },
///         reference.file.unwrap_or("<current file>"),
///         context.current_file().display(),
///         context.note_depth()
///     );
///
///     // Return emphasized text as replacement
///     vec![
///         Event::Start(Tag::Emphasis),
///         Event::Text(CowStr::from(reference.display())),
///         Event::End(TagEnd::Emphasis),
...
impl fmt::Debug for Exporter<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("WalkOptions")
            ...
            .field("missing_note_handler", &"<handler set>")
            .finish()
    }
}
...
impl<'a> Exporter<'a> {
    ...
    pub fn missing_note_handler(&mut self, handler: &'a MissingNoteHandler) -> &mut Self {
